<?php
    ...
    foreach($mc as $index => $item)
    {
        ...
        {
            case 0:
                echo "\t" . "\t" . "\t" . "\t" . '<p><li><b>' . str_replace("#", "", trim($item)) . ':</b> <button id="' . str_replace("#", "", trim($item)) . '">';
                break;
            case 1:
                $mc_ip = trim($item);
                break;
            ...
            case 4:
                ...
                if(ping($mc_ip, $mc_port) == "DOWN")
                    ...
                    if(ping($mc_ip, $mc_port) == "OK")
                        ...
                        $mc_color = "black";
                echo trim($item) . ' (<span class="' . $mc_color . '">' . ping($mc_ip, $mc_port) . '</span>)</p>' . PHP_EOL;
                break;
        }
    }
    echo "\t" . "\t" . "\t" . "\t" . '<p></ul></p>' . PHP_EOL;
